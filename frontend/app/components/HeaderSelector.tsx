...
export default function HeaderSelector({ selected, onChange }: HeaderSelectorProps) {
  ...
  return (
    <div className="flex h-full flex-col border border-line bg-[#FAFAFA]">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-line px-3 py-2">
        ...
        <button
          ...
          className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-wide text-muted transition hover:text-accent"
        >
      ...
      <div className="flex-1 overflow-y-auto p-2">
        <p className="mb-1.5 px-1 font-mono text-[9px] uppercase tracking-wider text-line">
          CRM Fields
        </p>
        ...
        {[...selected].filter((f) => !(CRM_FIELDS as readonly string[]).includes(f)).length > 0 && (
          <>
            <p className="mb-1.5 mt-3 px-1 font-mono text-[9px] uppercase tracking-wider text-line">
              Custom
            </p>
      ...
      <div className="border-t border-line p-2">
        <div className="flex gap-1">
          <input
            ...
            className="min-w-0 flex-1 border border-line bg-white px-2 py-1.5 font-mono text-[10px] text-ink placeholder-line outline-none focus:border-accent"
          />
          <button
            onClick={addCustom}
        ...
        <p className="mt-1.5 font-mono text-[9px] text-line">
